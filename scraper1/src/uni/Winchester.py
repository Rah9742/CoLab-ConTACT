...
class Winchester(University):

    def ScrapeForData(self, isRaw, depth, keywords):
        for i in range(len(keywords)):
            for y in range(depth):
                url = "https://winchester.elsevierpure.com/en/searchAll/advanced/?allThese=" + keywords[i] + "&exactPhrase=&or=&minus=&family=publications&doSearch=Search&slowScroll=true&resultFamilyTabToSelect=&page=" + str(y)
                # https://winchester.elsevierpure.com/en/searchAll/advanced/?allThese=Tech&exactPhrase=&or=&minus=&family=publications&doSearch=Search&slowScroll=true&resultFamilyTabToSelect=&page=0
        ...
        if (isRaw):
            self.OutputRaw("University of Winchester")
        else:
            self.OutputCSV("University of Winchester", "winchester")
