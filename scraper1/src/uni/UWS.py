...
class UWS(University):

    def ScrapeForData(self, isRaw, depth, keywords):
        for i in range(len(keywords)):
            for y in range(depth):
                url = "https://research-portal.uws.ac.uk/en/searchAll/advanced/?searchByRadioGroup=PartOfNameOrTitle&searchBy=PartOfNameOrTitle&allThese=" + keywords[i] + "&exactPhrase=&or=&minus=&family=publications&doSearch=Search&slowScroll=true&page=" + str(y)
                # https://research-portal.uws.ac.uk/en/searchAll/advanced/?searchByRadioGroup=PartOfNameOrTitle&searchBy=PartOfNameOrTitle&allThese=AI&exactPhrase=&or=&minus=&family=publications&doSearch=Search&slowScroll=true&page=0
        ...
        if (isRaw):
            self.OutputRaw("University of West Scotland")
        else:
            self.OutputCSV("University of West Scotland", "UWS")
